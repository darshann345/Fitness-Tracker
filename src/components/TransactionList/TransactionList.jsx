...
import UpdateDataForm from "../Forms/UpdateDataForm/UpdateDataForm";
...
export default function TransactionList({
  ...
  useEffect(() => {
    const startIndex = (currentPage - 1) * maxRecords; // 0 , 3
    const endIndex = Math.min(currentPage * maxRecords, transactions.length); // 3 , 4

    setCurrentTransactions([...transactions].slice(startIndex, endIndex));
    setTotalPages(Math.ceil(transactions.length / maxRecords)); // 4/3 = 1.33 => 2
